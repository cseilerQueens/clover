...
clover.plot <- function(dataDir, ncFileName) {
  ...
  for (v in names(nc$var)) {
    ...
    # Jump to the next variable if data array only contains NA:
    if (all(is.na(data))) {
      next
    }
    ...
    if (dimensions == "time") {
      clover.intFun.plot.time(ncFileName, data, v, long_name, units, time, dimensions, dim_values)
      matched <- TRUE
    } # OK
    # lat, time fw_pac_atl
    if (dimensions == "lat, time" & length(dim(data)) == 2) {
      # Bugfix for "idivide_pac_atl" and potentially other variables,
      # which has only one rather than two dimensions
      clover.intFun.plot.lat.time(ncFileName, data, v, long_name, units, time, dimensions, dim_values)
      matched <- TRUE
    } # OK
    ...
    if (dimensions == "mon, time") {
      clover.intFun.plot.mon.time(ncFileName, data, v, long_name, units, time, dimensions, dim_values)
      matched <- TRUE
    } # OK

    # doy, lat, time solarm
    if (dimensions == "doy, lat, time") {
      clover.intFun.plot.doy.lat.time(ncFileName, data, v, long_name, units, time, dimensions, dim_values)
      matched <- TRUE
    } # OK

    # lat, lev, time a_atl
    if (dimensions == "lat, lev, time") {
      clover.intFun.plot.lat.lev.time(ncFileName, data, v, long_name, units, time, dimensions, dim_values)
      matched <- TRUE
    } # OK

    # lat, mon, time ptrop
    if (dimensions == "lat, mon, time") {
      clover.intFun.plot.lat.mon.time(ncFileName, data, v, long_name, units, time, dimensions, dim_values)
      matched <- TRUE
    } # OK
    # latv, mon, time vabz
    ...
    if (dimensions == "lon, lat, time") {
      clover.intFun.plot.lon.lat.time(ncFileName, data, v, long_name, units, time, dimensions, dim_values)
      matched <- TRUE
    } # OK

    # doy, hour, lat, time solar
    if (dimensions == "doy, hour, lat, time") {
      clover.intFun.plot.doy.hour.lat.time(ncFileName, data, v, long_name, units, time, dimensions, dim_values)
      matched <- TRUE
    } # OK

    # lat, lev, month, time t_atl
    if (dimensions == "lat, lev, month, time") {
      clover.intFun.plot.lat.lev.month.time(ncFileName, data, v, long_name, units, time, dimensions, dim_values)
      matched <- TRUE
    } # OK

    # latv, lev, month, time hftz
    if (dimensions == "latv, lev, month, time") {
      clover.intFun.plot.lat.lev.month.time(ncFileName, data, v, long_name, units, time, dimensions, dim_values)
      matched <- TRUE
    } # OK, but results seem wrong for hftz

    # latv, zlevw, mon, time xz
    if (dimensions == "latv, zlevw, mon, time") {
      clover.intFun.plot.latv.zlev.mon.time(ncFileName, data, v, long_name, units, time, dimensions, dim_values)
      matched <- TRUE
    } # OK

    # latv1, levw, month, time opsi
    if (dimensions == "latv1, levw, month, time") {
      clover.intFun.plot.latv1.levw.month.time(ncFileName, data, v, long_name, units, time, dimensions, dim_values)
      matched <- TRUE
    } # OK
    ...
    # lon, lat, lev, time vol
    if (dimensions == "lon, lat, lev, time") {
      clover.intFun.plot.lon.lat.lev.time(ncFileName, data, v, long_name, units, time, dimensions, dim_values)
      matched <- TRUE
    } # OK

    # lon, lat, mon, time tam
    if (dimensions == "lon, lat, mon, time") {
      clover.intFun.plot.lon.lat.mon.time(ncFileName, data, v, long_name, units, time, dimensions, dim_values)
      matched <- TRUE
    } # OK
    ...
    # lon, latv, lev, month, time v
    if (dimensions == "lon, latv, lev, month, time") {
      clover.intFun.plot.lon.lat.lev.month.time(ncFileName, data, v, long_name, units, time, dimensions, dim_values)
      matched <- TRUE
    } # OK
    ...
    # lonu, lat, lev, month, time u
    if (dimensions == "lonu, lat, lev, month, time") {
      clover.intFun.plot.lon.lat.lev.month.time(ncFileName, data, v, long_name, units, time, dimensions, dim_values)
      matched <- TRUE
    } # OK

    if (!matched) {
      message <- paste(v, "was not plotted since no function for dimensions", dimensions)
      print(message)
    }
